...
let
  cfg = config.services.cory.tint2;
in {
  options.services.cory.tint2 = {
    enable = mkEnableOption "Enables tint2";
  };

  config = mkIf cfg.enable {
    home-manager.users.cory.home.file.".config/tint2/tint2_vol.sh" = {
      executable = true;
      text = ''
        #!/bin/sh
    ...
    home-manager.users.cory.home.packages = [ pkgs.tint2 ];
  };
}
